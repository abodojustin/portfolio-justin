...
const projectDetailsMap: Record<string, ProjectDetails> = {
    'portofolio-justin': { detailedDescription: "Ce portfolio a été construit avec Next.js pour un rendu côté serveur rapide, TypeScript pour la robustesse du code, et Tailwind CSS pour un design moderne et responsive. L'interactivité est gérée par Framer Motion. Le défi était de créer une expérience utilisateur fluide et un design qui reflète des compétences techniques avancées.", techStack: ["nextjs", "react", "typescript", "tailwind", "framermotion"], },
                        ...
                        <motion.div key={project.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.1 }}>
                            <div className="group h-full flex flex-col bg-card-bg p-4 rounded-xl border border-border-color transition-all duration-300 hover:border-accent/50 hover:scale-105">
                                <div className="relative w-full h-40 mb-4 rounded-lg overflow-hidden cursor-pointer" onClick={() => handleProjectClick(project)}>
                                    <Image src={`/projects/${project.name}.png`} alt={`Aperçu de ${project.name}`} layout="fill" objectFit="cover" onError={(e) => { e.currentTarget.src = '/projects/justin_profile.jpeg'; }} />
